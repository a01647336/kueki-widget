...
> Las pruebas se ejecutan con `npm test`. Cubren la lógica del frontend (utils, hooks, componentes), el cliente HTTP (`api.ts`), la integración del backend multiusuario (Supertest + **PostgreSQL en memoria (pg-mem)**) y los **escenarios de rechazo de compra** (`eligibility.test.ts`).
...
**Módulo probado**: `server/server.js` (con Supertest y **PostgreSQL en memoria (pg-mem)**)
...
> Usa `pg-mem` y re-siembra la base antes de cada prueba para aislar el estado.
